...
threshold is sharp: we exhibit a reflexive polytope with $7$ vertices
whose hypersurface has a single ordinary double point and admits no
smoothing.  At $\ell(F^\circ)=1$ the germs over a face with one interior
...
whose criterion constrains only the $\ell(F^\circ)=1$ faces: exactly
...
the corresponding role here: a single reflexive $4$-polytope whose
Calabi--Yau hypersurface is provably non-smoothable.  In both of those works the Minkowski data live on
...
per-face condition accounts for exactly $3{,}774$ of the $30{,}241$
...
datum for $(N,\sigma)$, with $\sigma$ strongly convex and
...
$\mathbb{R}^+ Q$ (the boundary-datum form of the definition, which is
what \cite[Thm.~6.1]{Petracci18} takes as input).

\begin{lemma}[deformation datum from a codimension-two face]\label{lem:datum}
...
\]
is a deformation datum for $(\ZZ^5, \tau)$ in the sense of
...
$(N_F,\sigma_F)$; write $\tilde\sigma_F = \tilde\tau \cap
(N_F\oplus\ZZ^r)_\RR$ for its extended cone (here $N_F$ is embedded in
$\ZZ^5$ by the height section $n \mapsto (n, u_F(n))$, and the summands
are translated into $N_F$, as is possible because the direction lattice
of $\operatorname{aff} F$ is saturated there).  Restrict the face family
to the chart at the face $G \preceq \tilde\tau$ lying over $\sigma_F$
(the cone of Step~2; that $G$ is a face of $\tilde\tau$ is seen from a
supporting functional assembled from the constants
$\langle v^\ast,\cdot\rangle|_{\operatorname{aff} Q_i}$, the functional
$v^\ast$ being affine-constant on each summand's translate).  On the
chart's torus the $j$-th trinomial of the family reads
\[
...
generic anticanonical $X$, given by trinomial equations in the Cox coordinates,
...
($T$ a standard triangle, whose cone $C(T)$ is the $\frac13(1,1,1)$ point,
...
non-smoothable, then deforms, through an explicit trinomial family, to a
...
Let $\Delta_N$ be the reflexive polytope of
Theorem~\textup{\ref{thm:B}}.  Then $\Delta_N$ has $7$ vertices and unit
...
exceptional locus a single rational curve $\gamma$ over $p$, and it is
projective because the MPCP subdivision is projective \cite{Batyrev94}.
If $A$ is ample on $\widehat X_N$ then $A \cdot \gamma > 0$, so
...
single node this forces $[\gamma] = 0$.  Hence $X_N$ admits no smoothing.
\end{proof}
...
number of $\widehat X \to X$ is $\rho(S) \ge 2$.  Since
...
divisors of $C(F)$, which generate its class group.  A class of
infinite order in $\operatorname{Cl}(C(F))$ stays of infinite order in
the analytic local class group at the point: divisor classes on the
punctured germ are computed on the $\CC^\ast$-bundle
$(\text{total space of } \omega_S) \smallsetminus S$ over the del
Pezzo surface $S$, whose Picard group is
$\operatorname{Pic}(S)/\ZZ K_S$ in the algebraic and the analytic
category alike, the same group as $\operatorname{Cl}(C(F))$.  Such a
class is therefore represented by an algebraic divisor that is
not $\mathbb{Q}$-Cartier at the point, so $X$ is not
...
We have tabulated this over the \emph{entire} Kreuzer--Skarke list: for
...
as the lattice length of the edge of $\Delta^\circ$ dual to it.  Our
enumeration finds exactly $198{,}849$ reflexive $4$-polytopes in the
