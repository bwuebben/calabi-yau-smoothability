...
$\ell(F^\circ)$ is the lattice length of the dual edge.  In a companion
paper we showed that $X$ admits no smoothing when some $C(F)$ has no
smoothing component.  Here we study the complementary case, in which every singular
...
unique non-smooth face, and unconditionally whenever every non-smooth
face is a unit parallelogram with $\ell(F^\circ)\ge2$; for several
...
companion paper.  For polytopes all of whose singular faces
are unit parallelograms we compare the resulting per-face criterion with
the Namikawa-criterion census of Batyrev--Kreuzer: at most one in eight
of their $30{,}241$ certified smoothings is explained face-locally, and
at least $26{,}467$ involve a cross-face homology relation.
\end{abstract}
...
per-face condition accounts for at most $3{,}774$ of the $30{,}241$
conifold polytopes that Batyrev--Kreuzer certify smoothable --- about one
...
\cite[Ch.~III, Example~9.8.5]{Hartshorne}.
