...
\hypersetup{pdfauthor={Bernd Johannes Wuebben},
  pdftitle={Deformations of toric pairs and the smoothing of Batyrev Calabi-Yau threefolds}}
...
\title[Deformations of toric pairs]{Deformations of toric pairs\\ and the
smoothing of Batyrev Calabi--Yau threefolds}
...
$\mathbb{Q}$-factorial there.  If such a cone is the only singular point,
its relation kernel is zero, and the later mixed smoothing criterion
rules out a smoothing.  Finally we compare
...
if it does.  For the following divisorial germs, the later mixed
criterion of \cite{paper5} also gives a nonsmoothability result:
...
applies.  Nevertheless, the mixed criterion of \cite{paper5}
shows that $X$ is not smoothable.
\end{proposition}
...
$\mathbb{Q}$-factorial.  Its relation kernel is zero, so the mixed
criterion of \cite{paper5} rules out a smoothing.  At the level of
germs, Gross's
...
non-smoothable (Prop.~\ref{prop:C}; \cite{paper5}) & smoothable (Thm~\ref{thm:A})\\
...
\section{The \texorpdfstring{$\ell(F^\circ) = 1$}{dual-edge length one} germs: where Friedman's and Gross's
...
The same restriction calculation also determines smoothability.
The boundary divisor classes above span $H^2(S;\mathbb Q)$, so
$H_2(S;\mathbb Q)\to H_2(\widehat X;\mathbb Q)$ is injective.
There is only one singular point.  Every selected root space is a
subspace of $H_2(S;\mathbb Q)$, hence its global relation kernel is
zero.  The criterion of \cite[Thm.~A]{paper5} therefore rules out a
smoothing for each local component.
\end{proof}
...
Proposition~\ref{prop:C2} shows that these Batyrev hypersurfaces do
not provide the primitive realization.  Their nonsmoothability follows
from the zero relation kernel and the mixed criterion of
\cite{paper5}, even though each possible germ is locally smoothable.
\end{remark}
...
Is a Batyrev Calabi--Yau threefold whose unique singular point has
germ $C(F)$, with $F$ of type \textup{(S)}, $i(F)\ge2$, and
$\ell(F^\circ)=1$, ever smoothable?
The case $i(F)=0$ is excluded by Theorem~\ref{thm:B2}; the case
$i(F)=1$ is excluded by Proposition~\ref{prop:C2} and the criterion
of \cite{paper5}.  For $i(F)\ge2$ the crepant resolution has a
reducible exceptional divisor, and these exact germs are not covered
by that criterion.
\end{question}

\begin{question}\label{q:multi}
Is Theorem~\ref{thm:Amulti}(ii) unconditional?  Can the
...
data of \cite{CHP24})?  Separately, is the semiuniversal base of a Batyrev
hypersurface with only type-\textup{(S)} faces always irreducible?  The
fibres of a face family need not be toric, so the construction cannot be
...
run over the full Kreuzer--Skarke list.  The later criterion of
\cite{paper5} answers this question for nodes and its specified smooth
del Pezzo cone germs, with the full local discriminant nonvanishing
conditions.
\end{question}
