...
1. Identification lemma bookkeeping (Lemma: chart identification) for the
   ...
   the segment rays and vice versa — so Petracci's trinomial restricts on
   the chart of the face to the Cayley binomial equation
   chi^{m1} - chi^{m0} = t.
2. Delta_N (paper 2 Thm B): reflexivity/faces/Hodge re-asserted, and the
   ten facet normals recomputed and compared with the list printed in
   the paper (which prints their negatives, in the >= -1 normalization).
3. Star subdivisions at the interior point are unimodular for the THREE
   i=1 type-(S) classes (diamond = P1xP1-cone, dP7 pentagon, dP6
   hexagon) — the subdivision lemma used in Prop C's proof.
"""
