...
$\cX\to\Delta$ a Gorenstein morphism after shrinking the disk, so the
relative dualizing sheaf $\omega_{\cX/\Delta}$ is invertible and
...
$c_1(\omega_{\cX/\Delta})$ to the corresponding exterior in $X_s$
vanishes.  On each Milnor piece $M_p$, it also vanishes: the invertible
sheaf $\omega_{\cX/\Delta}$ is trivial on a sufficiently small
total-space neighborhood of $(p,0)$.
...
Pezzo surface $S$; the homotopy sequence shows that $\pi_1(L_p)=0$
because its connecting map $\pi_2(S)\to\pi_1(S^1)=\ZZ$ is evaluation
against the primitive Euler class $K_S$ and hence is surjective.  The Mayer--Vietoris
...
because its $\omega$ is nontrivial.)  Constants and upper
semicontinuity give $h^0(\cO_{X_s})=1$ after shrinking, hence
$h^0(\omega_{X_s})=1$.  By Grauert's base-change theorem
\cite{Grauert60}, $\pi_*\omega_{\cX/\Delta}$ is then a line bundle on the shrunk
