...
so the deformation theory of the affine variety coincides with that of
the analytic germ $(Y,0)$, whose analytic miniversal deformation
...
vanishing locus is used, which is independent of choices), and $X_s$
...
both cases have one.  In the unimodular chart with coordinates
...
while $\chi(s)\neq0$ off the discriminant; hence $c(\nu)=0$,
contradicting the injectivity of the period pairing in
Lemma~\ref{lem:D1}.
\end{proof}
...
$(\gamma\cdot\delta_q)F$ by the perfect pairing.  At a cone point,
$H_3(M;\QQ)$ has rank one or two and its skew intersection form
vanishes (in rank one for trivial reasons, and in rank two because
$H_3(M;\QQ)$ is generated by boundary-supported tube classes with
disjoint collar representatives), so Lefschetz duality makes
$H_3(M)\to H_3(M,L)$ zero and $\partial\colon H_3(M,L)\to H_2(L)$
injective with image $\ker(H_2(L)\to H_2(M))=R_p$.
\end{proof}
...
family compactifies by the same $\Sb\cong S$, and Ehresmann for the
pair identifies the tube boundaries
$\partial\,\mathrm{Tube}(\Sb\subset V_b)$ with
$\partial\,\mathrm{Tube}(S_\infty\subset\text{projective cone})$
fiber-preservingly.  Both identifications of $\partial M_b$ with
$L_p$ are thus given by transport to $b=0$, where the two circle
fibrations in question are the two ends of the punctured cone,
carrying the \emph{same} $\CC^*$-orbit fibration over $S$; the radial
rescaling flow, which is tame for the quasi-homogeneous family (the
weighted radius scales monotonically along orbits), identifies the
radius-$\varepsilon$ boundary with the large-radius tube boundary of
the same fiber fibration-compatibly.  Assembling, the two projections
...
smoothing.  Define $K(X)$ as in Theorem~\ref{thm:B-intro}, with
$R_p=\ker(H_2(L_p;\QQ)\to H_2(M_p;\QQ))$ for the actual Milnor fibers
of the induced local smoothings.  Then:
\begin{enumerate}
...
Likewise, no formal deformation of $X^{\circ}$ meets all thirty local
smoothing loci.
\end{corollary}
...
analytic arc with the same property.  (The local components are read
...
\texttt{global\_section.sage}, with three further Sage certificates
